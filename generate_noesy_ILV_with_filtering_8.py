...
def process_noesy(noesy_path, sequence, min_seq_sep=1, max_seq_sep=None, target_atom_from_name="CA", target_atom_to_name="CA"):
    """
        ...
        if (aa_from in hydrop) and (aa_to in hydrop) and (atom_from == "H") and (atom_to == "H"):
            if seq_sep >= min_seq_sep and (max_seq_sep is None or seq_sep <= max_seq_sep):
                filtered.append({
                    ...
                    'seq_sep': seq_sep,
                    'atom_from': target_atom_from_name,
                    'atom_to': target_atom_to_name
                })
...
def create_config(a3m_path, noesy_path, output_path, min_seq_sep=1, max_seq_sep=None, target_atom_from_name="CA", target_atom_to_name="CA"):
    chain_id, seq = parse_a3m(a3m_path)
    constraints = process_noesy(noesy_path, seq, min_seq_sep=min_seq_sep, max_seq_sep=max_seq_sep, target_atom_from_name=target_atom_from_name, target_atom_to_name=target_atom_to_name)
    yaml_data = {
        'version': 1,
        'sequences': [{
            'protein': {
...
if __name__ == "__main__":
    ...
    create_config(
        ...
        target_atom_from_name="CA",
        target_atom_to_name="CA"
    )
